...
List_of_Methods = {'Second_Dual_Predictor','Second_Classic_Predictor'};
Marker = {'o','*','x'};
...
for method_number = 1:length(List_of_Methods)
    ...
    for agent = 1:Nagent    
        [ X2{agent}, Z2{agent}, mu2{agent}, X_sens{agent}, X_sens_tau{agent}, Z_sens{agent}, Z_sens_tau{agent}, mu_sens{agent}, mu_sens_tau{agent}, Store{method_number}.iterNT(agent) ] = NTSolve(Q{agent} + WeightNuclear*eye(Nvar), C{agent}, lambda, A{agent}, a{agent}, tau, tau, X{agent}, Z{agent}, mu{agent}, P{agent}, display_subproblems);
    end

    tol = tau/2;
    [ X, Z, mu, lambda ] = Center( Q, WeightNuclear, C, A, a, X, Z, mu, lambda, P, Cset, tau, tol );
    ...
    while tau > toltau || norm(res) > toltau
        ...
        delta_func{method_number}(tau_index) = sqrt(res.'*dlambda)/sqrt(tau);
...
% end
% legend({'Algorithm 2','Algorithm 2 w/o predictor'},'location','southwest')
...
for method_number = 1:length(List_of_Methods)
    ...
    %semilogy(delta_func{method_number},'linestyle',LS,'marker',Marker{method_number},'color','r');hold on
end
legend({'Algorithm 2','Algorithm 2 w/o predictor\qquad'},'interpreter','latex','fontsize',FS,'location','southwest')
for method_number = 1:length(List_of_Methods)
...
set(gca,'fontsize',FS)
legend({'Algorithm 2','Algorithm 2, w/o predictor\qquad'},'interpreter','latex','location','southeast')
